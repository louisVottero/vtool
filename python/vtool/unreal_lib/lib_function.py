...
class VetalaLib(object):
    ...
    def OutputRigControls(self, controller, library):
        ...
        branch = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_ControlFlowBranch', 'Execute', unreal.Vector2D(960.0, 96.0), 'Branch')
        greater = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_MathIntGreater', 'Execute', unreal.Vector2D(80.0, 176.0), 'Greater')
        if1 = controller.add_template_node('DISPATCH_RigVMDispatch_If(in Condition,in True,in False,out Result)', unreal.Vector2D(1136.0, -144.0), 'If')
        add_tag = controller.add_unit_node_from_struct_path('/Script/ControlRig.RigUnit_SetMetadataTag', 'Execute', unreal.Vector2D(1696.0, 256.0), 'Add Tag')
        for_each = controller.add_template_node('DISPATCH_RigVMDispatch_ArrayIterator(in Array,out Element,out Index,out Count,out Ratio)', unreal.Vector2D(1456.0, 256.0), 'For Each')
        from_string = controller.add_template_node('DISPATCH_RigDispatch_FromString(in String,out Result)', unreal.Vector2D(1488.0, 576.0), 'From String')
        set_control_visibility = controller.add_unit_node_from_struct_path('/Script/ControlRig.RigUnit_SetControlVisibility', 'Execute', unreal.Vector2D(3056.0, -608.0), 'Set Control Visibility')
        less_equal = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_MathIntLessEqual', 'Execute', unreal.Vector2D(2224.0, -704.0), 'Less Equal')
        num = controller.add_template_node('DISPATCH_RigVMDispatch_ArrayGetNum(in Array,out Num)', unreal.Vector2D(1808.0, -384.0), 'Num')
        branch1 = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_ControlFlowBranch', 'Execute', unreal.Vector2D(2432.0, -416.0), 'Branch')
        not_equals = controller.add_template_node('DISPATCH_RigVMDispatch_CoreNotEquals(in A,in B,out Result)', unreal.Vector2D(2048.0, -368.0), 'Not Equals')
        for_each1 = controller.add_template_node('DISPATCH_RigVMDispatch_ArrayIterator(in Array,out Element,out Index,out Count,out Ratio)', unreal.Vector2D(2528.0, -704.0), 'For Each')
        equals = controller.add_template_node('DISPATCH_RigVMDispatch_CoreEquals(in A,in B,out Result)', unreal.Vector2D(-400.0, -592.0), 'Equals')
        branch2 = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_ControlFlowBranch', 'Execute', unreal.Vector2D(112.0, -544.0), 'Branch')
        set_item_array_metadata = controller.add_template_node('DISPATCH_RigDispatch_SetMetadata(in Item,in Name,in NameSpace,in Value,out Success)', unreal.Vector2D(656.0, -592.0), 'Set Item Array Metadata')
        vetala_lib_get_item = controller.add_function_reference_node(library.find_function('vetalaLib_GetItem'), unreal.Vector2D(-288.0, 480.0), 'vetalaLib_GetItem')
        get_item_array_metadata = controller.add_template_node('DISPATCH_RigDispatch_GetMetadata(in Item,in Name,in NameSpace,in Default,out Value,out Found)', unreal.Vector2D(256.0, 416.0), 'Get Item Array Metadata')
        num1 = controller.add_template_node('DISPATCH_RigVMDispatch_ArrayGetNum(in Array,out Num)', unreal.Vector2D(240.0, -96.0), 'Num')
        greater1 = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_MathIntGreater', 'Execute', unreal.Vector2D(432.0, -128.0), 'Greater')
        and1 = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_MathBoolAnd', 'Execute', unreal.Vector2D(2240.0, -224.0), 'And')
        int_to_name = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_MathIntToName', 'Execute', unreal.Vector2D(144.0, -304.0), 'Int to Name')
        concat = controller.add_unit_node_from_struct_path('/Script/RigVM.RigVMFunction_NameConcat', 'Execute', unreal.Vector2D(416.0, -320.0), 'Concat')
        ...
        graph.add_link(branch1,'Completed',return1,'ExecuteContext',controller)
        graph.add_link(branch2,'Completed',branch,'ExecuteContext',controller)
        ...
        graph.add_link(less_equal,'Result',set_control_visibility,'bVisible',controller)
        ...
        graph.add_link(not_equals,'Result',and1,'A',controller)
        ...
        graph.set_pin(greater, 'B', '2', controller)
        graph.set_pin(add_tag, 'NameSpace', 'Self', controller)
        graph.set_pin(less_equal, 'B', '1', controller)
