...
class PoseBase(PoseGroup):
    ...
    def _create_mirror_mesh(self, target_mesh):
        ...
        mirror_group = cmds.group(em=True, n=core.inc_name('corretive_mirror_group'))  # TODO: Possible typo here.
    ...
    def disconnect_weight_outputs(self):
        ...
        for output in outputs:
            ...
            if cmds.nodeType(node) == 'multiplyDivide' and cmds.isConnected('%s.enable' % self.pose_control,
                                                                            '%s.input2X' % node):
                continue
