...
def create_scene_script_jobs():
    ...
    job_new_scene = cmds.scriptJob(
        event=['NewSceneOpened', 'from vtool.maya_lib import ui_core;ui_core.emit_new_scene_signal()'], protected=False)
    job_open_scene = cmds.scriptJob(
        event=['SceneOpened', 'from vtool.maya_lib import ui_core;ui_core.emit_open_scene_signal()'], protected=False)
    job_read_scene = cmds.scriptJob(
        ct=['readingFile', 'from vtool.maya_lib import ui_core;ui_core.emit_read_scene_signal()'], protected=False)
    job_selection_changed = cmds.scriptJob(
        event=['SelectionChanged', 'from vtool.maya_lib import ui_core;ui_core.emit_selection_change_signal()'],
        protected=False)
