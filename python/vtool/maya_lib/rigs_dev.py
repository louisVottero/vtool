...
class StickyRig(rigs.JointRig):
 ...
 \
    ...
    def create_zip(self, attribute_control, increment, start, end, end_value=1):
        ...
        anim.quick_driven_key('%s.zipL' % attribute_control, '%s.stick' % self.zip_controls[increment][0][0],
                              [start, end], [0, end_value])
        anim.quick_driven_key('%s.zipL' % attribute_control, '%s.stick' % self.zip_controls[increment][0][1],
                              [start, end], [0, end_value])

        if left_over_value:
            anim.quick_driven_key('%s.zipR' % attribute_control, '%s.stick' % self.zip_controls[increment][0][0],
                                  [start, end], [0, left_over_value])
            anim.quick_driven_key('%s.zipR' % attribute_control, '%s.stick' % self.zip_controls[increment][0][1],
                                  [start, end], [0, left_over_value])
        ...
        anim.quick_driven_key('%s.zipR' % attribute_control,
                              '%s.stick' % self.zip_controls[increment][right_increment][0], [start, end],
                              [0, end_value])
        anim.quick_driven_key('%s.zipR' % attribute_control,
                              '%s.stick' % self.zip_controls[increment][right_increment][1], [start, end],
                              [0, end_value])

        if left_over_value:
            anim.quick_driven_key('%s.zipL' % attribute_control,
                                  '%s.stick' % self.zip_controls[increment][right_increment][0], [start, end],
                                  [0, left_over_value])
            anim.quick_driven_key('%s.zipL' % attribute_control,
                                  '%s.stick' % self.zip_controls[increment][right_increment][1], [start, end],
...
class StickyLipRig(StickyRig):
    ...
    def _cluster_curves(self):
        ...
        cmds.parent(self.clusters_top, self.clusters_btm, self.clusters_guide_top, self.clusters_guide_btm,
                    cluster_group)
        cmds.parent(cluster_group, self.setup_group)

    def _create_curve_joints(self):

        self.top_tweak_joints, self.top_joint_group, top_controls = rigs_util.create_joints_on_curve(self.top_curve,
                                                                                                     len(
                                                                                                         self.top_joints),
                                                                                                     self.description)
        self.btm_tweak_joints, self.btm_joint_group, btm_controls = rigs_util.create_joints_on_curve(self.btm_curve,
                                                                                                     len(
                                                                                                         self.btm_joints),
                                                                                                     self.description)
    ...
    def _create_corner_controls(self):
        ...
        for side in ['L', 'R']:
            ...
            if side == 'L':
                ...
                self.main_controls[0][0] = cmds.rename(self.main_controls[0][0],
                                                       self.main_controls[0][0].replace('CNT_', 'ctrl_'))
                cmds.delete('%sShape' % self.main_controls[0][0])
                self.main_controls[1][0] = cmds.rename(self.main_controls[1][0],
                                                       self.main_controls[1][0].replace('CNT_', 'ctrl_'))
            ...
            if side == 'R':
                ...
                self.main_controls[2][0] = cmds.rename(self.main_controls[2][0],
                                                       self.main_controls[2][0].replace('CNT_', 'ctrl_'))
                cmds.delete('%sShape' % self.main_controls[2][0])
                self.main_controls[3][0] = cmds.rename(self.main_controls[3][0],
                                                       self.main_controls[3][0].replace('CNT_', 'ctrl_'))
    ...
    def insert_follows(self, joints, percent=0.5, create_locator=True):
        ...
        for inc in range(0, joint_count):
            ...
            for axis in all_axis:
                attr.connect_multiply('%s.translate%s' % (self.drivers[inc], axis), '%s.translate%s' % (driver, axis),
                                      percent, skip_attach=True)

                attr.connect_multiply('%s.translate%s' % (self.locators[inc], axis), '%s.translate%s' % (locator, axis),
                                      percent, skip_attach=True)
...
class EyeLidSphereRig(rigs.BufferRig):
    ...
    def create(self):
        ...
        joints, group = create_joint_slice(center_joint, '%s1_%s' % (self.description, self.side), radius=self.radius,
                                           sections=self.sections, axis=self.axis)
...
class EyeLidSphereRig2(rigs.BufferRig):
    ...
    def _create_nurbs_sphere(self):
        ...
        self.surface = cmds.sphere(ch=False, o=True, po=False, ax=[0, 1, 0], radius=self.radius, nsp=4,
                                   n='surface_%s' % core.inc_name(self._get_name()))[0]
                ...
                if self.first_folicle and inc2 != len(folicles) - 1:
                    ...
                    if reverse and inc2 != len(folicles) - 1:
                        plus = cmds.createNode('plusMinusAverage',
                                               n=core.inc_name(self._get_name('plusMinusAverage', 'comboBtm')))

                        attr.connect_multiply('%s.parameterV' % self.first_folicle, '%s.input1D[0]' % plus,
                                              multiply_value)
                        attr.connect_multiply('%s.parameterV' % self.last_folicle, '%s.input1D[1]' % plus,
                                              (1 - multiply_value))
                    ...
                    if not reverse:
                        ...
                        attr.connect_multiply('%s.outValue' % remap_front, '%s.inputValue' % remap_front2,
                                              multiply_value)
                        ...
                        plus = cmds.createNode('plusMinusAverage',
                                               n=core.inc_name(self._get_name('plusMinusAverage', 'combo')))
...
class MouthTweakers(rigs.Rig):
    ...
    def _create_joints_with_control_on_curve(self, curve, section_count=5, control=False):
        ...
        middle_joint, middle_aim, middle_xform, middle_control, middle_aim_control, middle_xform_control = self._create_joint(
            curve, length / 2.0, control=True)
        ...
        for inc in range(0, sections / 2):
            ...
            joint1, aim1, xform1, control1, aim_control1, xform_control1 = self._create_joint(curve, start_offset,
                                                                                              control=True)
            joint2, aim2, xform2, control2, aim_control2, xform_control2 = self._create_joint(curve, end_offset,
                                                                                              control=True)
...
class WorldStickyRig(rigs.JointRig):
    ...
    def create_zip(self, attribute_control, increment, start, end, end_value=1):
        ...
        anim.quick_driven_key('%s.zipL' % attribute_control, '%s.stick' % left_top_control, [start, end],
                              [0, end_value])
        anim.quick_driven_key('%s.zipL' % attribute_control, '%s.stick' % left_btm_control, [start, end],
                              [0, end_value])

        if left_over_value:
            anim.quick_driven_key('%s.zipR' % attribute_control, '%s.stick' % left_top_control, [start, end],
                                  [0, left_over_value])
            anim.quick_driven_key('%s.zipR' % attribute_control, '%s.stick' % left_btm_control, [start, end],
                                  [0, left_over_value])
        ...
        anim.quick_driven_key('%s.zipR' % attribute_control, '%s.stick' % right_top_control, [start, end],
                              [0, end_value])
        anim.quick_driven_key('%s.zipR' % attribute_control, '%s.stick' % right_btm_control, [start, end],
                              [0, end_value])

        if left_over_value:
            anim.quick_driven_key('%s.zipL' % attribute_control, '%s.stick' % right_top_control, [start, end],
                                  [0, left_over_value])
            anim.quick_driven_key('%s.zipL' % attribute_control, '%s.stick' % right_btm_control, [start, end],
                                  [0, left_over_value])
    ...
    def create_follow(self, follow_transform, increment, value):
        ...
        if increment != 'corner':
            ...
            if len(locators) > 1:
                ...
                follow_top = space.create_multi_follow([self.follower_group, follow_transform], top_locator2,
                                                       top_locator2, value=value)
                follow_btm = space.create_multi_follow([self.follower_group, follow_transform], btm_locator2,
                                                       btm_locator2, value=1 - value)

                self._rename_followers(follow_top, 'top')
                self._rename_followers(follow_btm, 'btm')

        if increment == 'corner':
            space.create_multi_follow([self.follower_group, follow_transform], self.corner_xforms[0],
                                      self.corner_xforms[0], value=value)
            space.create_multi_follow([self.follower_group, follow_transform], self.corner_xforms[1],
                                      self.corner_xforms[1], value=value)
...
class BackLeg(rigs.BufferRig):
    ...
    def _duplicate_joints(self):
        ...
        ik_chain = [self.offsetGuideChain[0], self.offsetGuideChain[1], self.offset1Chain[1], self.offset2Chain[1],
                    self.ikGuideChain[-1]]
...
class FrontLeg(rigs.BufferRig):
    ...
    def _duplicate_joints(self):
        ...
        ik_chain = [self.offsetGuideChain[0], self.offsetGuideChain[1], self.offsetGuideChainBtm[1],
                    self.ikGuideChain[-1]]
...
class IkFrontLegRig(rigs.IkAppendageRig):
    ...
    def _create_pole_vector(self):
        ...
        if self.create_twist:
            ...
            space.create_multi_follow([self.off_offset_locator, self.offset_locator], self.twist_guide_ik,
                                      self.btm_control, attribute_name='autoTwist', value=0)
    ...
    for inc in range(0, (sections / 2)):
        ...
        for dup in [dup_pos, dup_neg]:
            ...
            for inc in range(0, (sections / 2) + 1):
                ...
                multiply = attr.connect_multiply('%s.rotate%s' % (dup, axis), '%s.rotate%s' % (dup3, axis),
                                                 value_offset)
