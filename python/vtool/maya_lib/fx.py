...
def export_maya_cache(geo, name='maya_cache', dirpath=''):
    ...
    cmds.cacheFile(f=name, format='OneFile', points=found_shapes, dir=folder, ws=True, sch=True, st=min_value,
                   et=max_value)
...
def import_maya_cache(geo, name='maya_cache', dirpath='', source_namespace=None):
    ...
    for geo_name in found_shapes:
        ...
        cache_file = cmds.cacheFile(attachFile=True, fileName=name, dir=folder, ia='%s.inp[0]' % deformer[0],
...
def export_alembic(root_node, name, dirpath=None, auto_sub_folders=True, min_value=None, max_value=None):
    ...
    alembic_command = 'AbcExport -j "-frameRange %s %s -stripNamespaces' \
                      ' -uvWrite -worldSpace -writeVisibility -dataFormat' \
                      ' ogawa %s %s -file %s";' % (min_value, max_value, root_node_str, attr_node_str, filename)
...
def add_follicle_to_curve(curve, hair_system=None, switch_control=None, attribute_name='dynamic', blendshape=True,
                          out_curve_parent=None, attach_mesh=None):
    """
...
class CMuscle(object):
    ...
    def create_attributes(self, node=None):
        ...
        attributes = ['jiggle', 'jiggleX', 'jiggleY', 'jiggleZ', 'jiggleImpact', 'jiggleImpactStart',
