...
class YetiJob(MayaJob):
    ...
    def _get_command(self):
        command = "cmds.pgYetiCommand(yeti_node, writeCache=cache_path, range=(%s, %s), samples=%s)" % (
        self.in_value, self.out_value, self.samples)
...
class AlembicJob(MayaJob):
    ...
    def _get_command(self):
        command = "cmds.AbcExport( j = '-frameRange {} {} -stripNamespaces -uvWrite -worldSpace -writeVisibility -dataFormat ogawa -root %s -file %s' % (node, cache_path))".format(
...
class MayaCacheJob(MayaJob):
    ...
    def _get_command(self):
        command = "cmds.cacheFile(f=output_name,format='OneFile', points = nodes, dir = cache_path, ws = True, sch = True, st = %s, et = %s)" % (
        self.in_value, self.out_value)
        return command
