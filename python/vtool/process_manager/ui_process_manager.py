...
class ProcessManagerWindow(qt_ui.BasicWindow):
    ...
    def sizeHint(self):
        ...
        if util.in_unreal:
            ...
            return qt.QtCore.QSize(util.scale_dpi(400), util.scale_dpi(500))
    ...
    def _process(self, last_inc=None):
        ...
        for inc in range(start, script_count):
            ...
            if not skip:
                ...
                self.code_widget.script_widget.code_manifest_tree.set_process_data(self.process.runtime_values,
                                                                                   self.process._put)
    ...
    def _process_children(self, item):
        ...
        if children:
            ...
            result = qt_ui.get_comment(self,

                                       'Found process children checked on:\n\n'
                                       'Hit Ok to auto build children first.\n\n'
                                       'Hit Cancel to process only the current process.\n\n\n'
                                       'Add comment to the auto build? ',

                                       'Children Checked', comment_text='Auto Build')
