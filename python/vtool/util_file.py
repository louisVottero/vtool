...
def delete_pyc(python_script):
    ...
    if not python_script.endswith('.py'):
        util.warning('Could not delete pyc file for %s.'
                     ' Be careful not to run this command on files that are not .py extension.' % script_name)
        return
...
def remove_modules_at_path(path):
    util.show('Removing modules at path: %s' % path)
    ...
    for module in modules_to_pop:
        util.show('Removing module: %s' % module)
